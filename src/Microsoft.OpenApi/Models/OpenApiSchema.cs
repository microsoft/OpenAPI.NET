...
namespace Microsoft.OpenApi.Models
{
    ...
    public class OpenApiSchema : IOpenApiSerializable, IOpenApiReferenceable, IEffective<OpenApiSchema>, IOpenApiExtensible
    {
        ...
        public OpenApiSchema(OpenApiSchema schema)
        {
            ...
            AllOf = schema?.AllOf != null ? new List<OpenApiSchema>(schema?.AllOf) : schema?.AllOf;
            OneOf = schema?.OneOf != null ? new List<OpenApiSchema>(schema?.OneOf) : schema?.OneOf;
            AnyOf = schema?.AnyOf != null ? new List<OpenApiSchema>(schema?.AnyOf) : schema?.AnyOf;
            Not = new(schema?.Not);
            Required = schema?.Required != null ? new HashSet<string>(schema?.Required) : schema?.Required;
            Items = new(schema?.Items);
            MaxItems = schema?.MaxItems;
            MinItems = schema?.MinItems;
            UniqueItems = schema?.UniqueItems;
            Properties = schema?.Properties != null ? new Dictionary<string, OpenApiSchema>(schema?.Properties) : schema?.Properties;
            MaxProperties = schema?.MaxProperties;
            MinProperties = schema?.MinProperties;
            AdditionalPropertiesAllowed = schema?.AdditionalPropertiesAllowed ?? false;
            ...
            Enum = schema?.Enum != null ? new List<IOpenApiAny>(schema?.Enum) : schema?.Enum;
            Nullable = schema?.Nullable ?? false;
            ExternalDocs = new(schema?.ExternalDocs);
            Deprecated = schema?.Deprecated ?? false;
            Xml = new(schema?.Xml);
            UnresolvedReference = schema?.UnresolvedReference ?? false;
