...
public class OpenApiDynamicRefTests
{
    ...
    public async Task DynamicAnchorInResponseHeaderSchemaIsRegistered()
    {
        ...
        // exercises the response.Headers leaf, which (like parameter.Content and header.Content)
        // is a schema-bearing location that the anchor walk must cover.
        var yaml =
        """
        openapi: 3.2.0
        info:
    ...
    [Fact]
    public async Task DynamicAnchorInMediaTypeItemSchemaIsRegistered()
    {
        // A $dynamicAnchor declared inside a media type's itemSchema (an OAS 3.2 schema-bearing
        // leaf distinct from `schema`) must be registered, so a $dynamicRef resolves to it.
        var yaml =
        """
        openapi: 3.2.0
        info:
        ...
        Assert.Single(doc.Workspace.GetDynamicAnchorCandidates(doc, "entry"));

        var stream = doc.Components.Schemas["Stream"];
        var firstRef = Assert.IsType<OpenApiSchemaReference>(stream.Properties["first"]);
        Assert.True(firstRef.Reference.IsDynamicRefOnly);
        Assert.NotNull(firstRef.Target);

        var resolved = Assert.IsType<OpenApiSchemaReference>(firstRef.Target);
        Assert.Same(doc.Components.Schemas["Entry"], resolved.Target);
    }
    ...
    [Fact]
    public async Task DynamicAnchorInReusableMediaTypeIsRegistered()
    {
        // A $dynamicAnchor declared inside a reusable media type's schema (components/mediaTypes,
        // an OAS 3.2 component) must be registered, so a $dynamicRef resolves to it.
        var yaml =
        """
        openapi: 3.2.0
        info:
        ...
        Assert.Single(doc.Workspace.GetDynamicAnchorCandidates(doc, "widget"));

        var payload = doc.Components.Schemas["Payload"];
        var innerRef = Assert.IsType<OpenApiSchemaReference>(payload.Properties["inner"]);
        Assert.True(innerRef.Reference.IsDynamicRefOnly);
        Assert.NotNull(innerRef.Target);

        var resolved = Assert.IsType<OpenApiSchemaReference>(innerRef.Target);
        Assert.Same(doc.Components.Schemas["Widget"], resolved.Target);
    }
