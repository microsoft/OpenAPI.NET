...
namespace Microsoft.OpenApi.Readers.Tests.V32Tests
{
    public class OpenApiDocumentTests
    {
        ...
        {
            // Arrange && Act
            var actual = await OpenApiDocument.LoadAsync("V32Tests/Samples/OpenApiDocument/documentWithReusablePaths.yaml", SettingsFixture.ReaderSettings, token: TestContext.Current.CancellationToken);
        ...
        public async Task ParseDocumentWithExampleInSchemaShouldSucceed()
        {
            ...
            actual.Document.SerializeAsV32(writer);
  ...
  properties:
    ...
    prop3:
      type: string";
            ...
            var actualMediaType = await mediaType.SerializeAsYamlAsync(OpenApiSpecVersion.OpenApi3_2, TestContext.Current.CancellationToken);
        ...
        {
            var path = Path.Join(SampleFolderPath, "documentWith32Properties.yaml");
        ...
        {
            // Arrange && Act
            var path = Path.Join(SampleFolderPath, "documentWithSelfProperty.yaml");
            var result = await OpenApiDocument.LoadAsync(path, SettingsFixture.ReaderSettings, token: TestContext.Current.CancellationToken);
        ...
        {
            // Arrange && Act
            var path = Path.Join(SampleFolderPath, "documentWithSelfProperty.json");
            var result = await OpenApiDocument.LoadAsync(path, SettingsFixture.ReaderSettings, token: TestContext.Current.CancellationToken);
