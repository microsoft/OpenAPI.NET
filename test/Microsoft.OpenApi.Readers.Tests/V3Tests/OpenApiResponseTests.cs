...
namespace Microsoft.OpenApi.Readers.Tests.V3Tests
{
    ...
    public class OpenApiResponseTests
    {
        ...
        {
            var result = await OpenApiDocument.LoadAsync(Path.Join(SampleFolderPath, "responseWithSummary.yaml"), SettingsFixture.ReaderSettings, token: TestContext.Current.CancellationToken);
        ...
        {
            var result = await OpenApiDocument.LoadAsync(Path.Join(SampleFolderPath, "responseWithSummaryExtension.yaml"), SettingsFixture.ReaderSettings, token: TestContext.Current.CancellationToken);
